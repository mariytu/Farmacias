...
namespace FarmaciasAPI.Repositories
{
    public interface IFarmaciasRepository
    {
        ...
        Task<ICollection<FarmaciasDTO>> GetFarmaciasPorComunaIdLocalAsync(int comunaId, string local);
    }
    ...
    public class FarmaciasRepository : IFarmaciasRepository
    {
        ...
        public async Task<ICollection<FarmaciasDTO>> GetFarmaciasPorComunaIdLocalAsync(int comunaId, string local)
        {
            #region Validacion de campos obligatorios
            if (comunaId == 0)
                throw new ArgumentNullException(string.Format("El campo {0} no puede ser 0.", "comunaId"));
            if (string.IsNullOrEmpty(local))
                throw new ArgumentNullException(string.Format("El campo {0} no puede ser nulo o vacío.", "local"));
            #endregion
            ...
            // Se filtran las farmacias obtenidas por la comuna y nombre de local. Para el local el string solo debe contener parte del local
            return farmaciasResponse.Where(x => x.ComunaFK == comunaId && x.LocalNombre.ToLower().Contains(local.ToLower())).Select(x => new FarmaciasDTO(x)).ToList();
        }
    }
}
